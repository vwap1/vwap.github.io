...
input  int    TakeProfit = 500; // Take Profit (default : 500)
input  int    StopLoss   = 250; // Stop Loss (default : 250)
...
input  int    LockProfitAfter = 100; // Target Points to Lock Profit (default : 100)
input  int    ProfitLock      = 60;  // Profit To Lock (default : 60)

sinput string note3 = ""; // -=[ TRAILING STOP SETTINGS ]=-
input  ENUM_TRAILINGSTOP_METHOD TrailingStopMethod = TS_STEP_BY_STEP; // Trailing Method
input  int    TrailingStop = 50; // Trailing Stop (default : 50)
input  int    TrailingStep = 10; // Trailing Stop Step (default : 10)

sinput string note4 = ""; // -=[ OTHER SETTINGS ]=-
input  ENUM_CHARTSYMBOL ChartSymbolSelection = CurrentChartSymbol;; // AllOpenOrder
...
bool RZ_TrailingStop(int TiketOrder,int JumlahPoin,int Step=1,ENUM_TRAILINGSTOP_METHOD Method=TS_STEP_DISTANCE)
{
   ...
   {
      if (CurrentSL<OrderOpenPrice()) CurrentSL=OrderOpenPrice();
